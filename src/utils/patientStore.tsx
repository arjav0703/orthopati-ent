...
interface PatientContextType {
  patients: Patient[];
  addPatient: (
    patient: Omit<Patient, "id" | "createdAt" | "visits">,
  ) => Promise<string>;
  getPatient: (id: string) => Promise<Patient | undefined>;
  updatePatient: (
    id: string,
    data: Partial<Omit<Patient, "id" | "createdAt" | "visits">>,
  ) => Promise<boolean>;
...
export const PatientProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addPatient = async (
    patientData: Omit<Patient, "id" | "createdAt" | "visits">,
  ): Promise<string> => {
    try {
      const response = await fetch("/api/patients", {
        method: "POST",
  ...
  const addVisit = async (
    ...
    try {
      ...
      // Update local state
      setPatients((prevPatients) => {
        const patientIndex = prevPatients.findIndex(
