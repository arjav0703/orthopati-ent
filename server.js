...
app.get('/api/patients', async (req, res) => {
  try {
    ...
    const enrichedPatients = await Promise.all(patients.map(async (patient) => {
      const [visits] = await pool.execute('SELECT * FROM visits WHERE patientId = ?', [patient.id]);
      
      // For each visit, get images
      const enrichedVisits = await Promise.all(visits.map(async (visit) => {
        const [images] = await pool.execute('SELECT * FROM visit_images WHERE visitId = ?', [visit.id]);
        return {
          ...visit,
...
app.put('/api/patients/:id', async (req, res) => {
  try {
    ...
    await pool.execute(`UPDATE patients SET ${setClause} WHERE id = ?`, [...values, id]);
